...
```bash
fluentci gh init -t bazel_pipeline
```

Or, if you already have a `.fluentci` folder (generated from `fluentci init -t bazel`) in your project:
  ...
  test:
    ...
    steps:
      ...
      - name: Run Dagger Pipelines
        run: fluentci run bazel_pipeline
